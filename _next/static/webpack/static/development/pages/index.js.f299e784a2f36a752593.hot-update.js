...
function parserScriptCau(cau, dict) {
  ...
  for (j = 0; j < cacDoan.length; j++) {
    ...
    if (cacDoan[j]) {
      ...
      textScripts = textScripts.concat(word);
    }
  }
    ...
    value: function transToHanViet() {
      ...
      for (i = 0; i < cacCau.length; i++) {
        ...
        if (cacCau[i]) {
          ...
          if (cau[cau.length - 1]) cau[cau.length - 1]['dot'] = cacCau[i].substr(cacCau[i].length - 1);
...
//# sourceMappingURL=index.js.f299e784a2f36a752593.hot-update.js.map